...
async function isRevoked(jti: string): Promise<boolean> {
  ...
  if (!res.ok) {
    ...
    console.warn("[Core JWT] isRevoked failed:", res.status, text);
  ...
  if (revoked) console.warn("[Core JWT] Токен отозван, jti:", jti);
  return revoked;
}
...
export async function verifyBearerToken(
  ...
  if (!authorization?.startsWith("Bearer ")) {
    console.warn(
      "[Core JWT] Нет Bearer в Authorization:",
