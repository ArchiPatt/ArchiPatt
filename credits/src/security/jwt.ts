...
const JWKS_URL = `${env.authIssuer}/jwks`;
...
async function isRevoked(jti: string): Promise<boolean> {
  ...
  if (!res.ok) {
    ...
    console.warn("[Credits JWT] isRevoked failed:", res.status, text);
  ...
  if (revoked) console.warn("[Credits JWT] Токен отозван, jti:", jti);
  return revoked;
}
...
export async function verifyBearerToken(
  ...
  const expectedIssuer = env.authIssuer;

  if (!authorization?.startsWith("Bearer ")) {
    console.warn(
      "[Credits JWT] Нет Bearer в Authorization:",
