...
export class Migration1771762300000 implements MigrationInterface {
  name = "Migration1771762300000";
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const interestRateCol = pickColumnName(tariffCols, ["interest_rate", "interestRate"]);
    const billingCol = pickColumnName(tariffCols, ["billing_period_days", "billingPeriodDays"]);
    ...
    const accountIdCol = pickColumnName(creditCols, ["account_id", "accountId"]);
    const tariffIdCol = pickColumnName(creditCols, ["tariff_id", "tariffId"]);
    const principalCol = pickColumnName(creditCols, ["principal_amount", "principalAmount"]);
    const outstandingCol = pickColumnName(creditCols, ["outstanding_amount", "outstandingAmount"]);
    const issuedAtCol = pickColumnName(creditCols, ["issued_at", "issuedAt"]);
    const dueAtCol = pickColumnName(creditCols, ["next_payment_due_at", "nextPaymentDueAt"]);
    const closedAtCol = pickColumnName(creditCols, ["closed_at", "closedAt"]);

    const paymentCreditIdCol = pickColumnName(paymentCols, ["credit_id", "creditId"]);
    const paymentTypeCol = pickColumnName(paymentCols, ["payment_type", "paymentType"]);
    const performedByCol = pickColumnName(paymentCols, ["performed_by", "performedBy"]);
    const performedAtCol = pickColumnName(paymentCols, ["performed_at", "performedAt"]);
    ...
    await queryRunner.query(
      ...
      ["55555555-5555-4555-8555-555555555555", "QA Base Tariff", "0.0125", 30, true]
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DELETE FROM "${paymentTable}" WHERE "id" = $1`,
      ["77777777-7777-4777-8777-777777777770"]
    );
    await queryRunner.query(
      `DELETE FROM "${creditTable}" WHERE "id" = $1`,
      ["66666666-6666-4666-8666-666666666666"]
    );
    await queryRunner.query(
      `DELETE FROM "${tariffTable}" WHERE "id" = $1`,
      ["55555555-5555-4555-8555-555555555555"]
    );
  }
}
